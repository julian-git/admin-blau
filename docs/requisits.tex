...
\item \falta Quan es doni d'alta una família, avisar a tots els responsables de que se'ls ha agrupat en una família. 
...
\begin{itemize}
\item Completar la taula Families i les seves taules auxiliars
...
\item[R13] \falta Volem un llistat d'assistents a una activitat?? 

\item[R14] \complert Les quotes poden tenir una periodicitat diferent i un import diferent per cada casteller. 

\begin{itemize}
\item \complert Les periodicitats es definiran en mesos, i estaran predefinides en el sistema (trimestral, semestral, anual).  

\item \complert S'han de poder afegir noves periodicitats en un futur (encara que sigui directament a la BD) 

\item \complert Les periodicitats poden tenir un mes de cobrament (ex: anual, però cobrar al juliol). 
\end{itemize}
